...
class BodyMetricsFullBody extends Component {
  constructor() {
    super();
  ...
  getData = async (type) => {
    ...
    Object.keys(data.result).map((key, index) => {
      if (type === "weight") {
        if (this.props.unit === "standard") {
          array.push({ date: key, value: data.result[key] * 2.20462 });
        } else {
          array.push({ date: key, value: data.result[key] });
        }
      } else {
        if (this.props.unit === "standard") {
          array.push({ date: key, value: data.result[key] / 2.54 });
        } else {
          array.push({ date: key, value: data.result[key] });
        }
      }
...
*/
  ...
  onCalendarDonePress = async () => {
    ...
    if (
      typeof this.state.start.hasOwnProperty("dateString") &&
      ...
      <View style={{ backgroundColor: "rgb(255,255,255)", flex: 1 }}>
        ...
        <ScrollView>
          ...
          {this.props.gender === "Male" ? (
            ...
            >
              <Image
                source={require("../resources/icon/maleFullBody.png")}
            ...
            >
              <Image
                source={require("../resources/icon/femaleFullBody.png")}
                style={{ alignSelf: "center" }}
              />
            </View>
          )}
          ...
          this.state.heightData !== null &&
          this.state.heightData.length === 0 ? (
            <View>
              <Image
                source={require("../resources/icon/no_measurements.png")}
                style={{ alignSelf: "center", marginTop: 96 }}
              />
                  ...
                  >
                    <View>
                      <TouchableWithoutFeedback
                        onPress={() =>
                          this.setState({
                            isWeightCollapsed: !this.state.isWeightCollapsed,
                          })
                        }
                      >
                        <View
                          ...
                          <View>
                            ...
                            <Text style={styles.cardText}>
                              {this.props.unit === "standard"
                                ? parseFloat(
                                    this.state.weightCurrent * 2.20462
                                  ).toFixed(1)
                                : this.state.weightCurrent}{" "}
                              <Text style={[styles.cardText, { fontSize: 14 }]}>
                                {this.props.unit === "standard" ? "lbs" : "kg"}
                              </Text>
                            </Text>
                          </View>
                          ...
                          >
                            ...
                            {this.state.weightComparison !== 0 && (
                              <Text style={styles.cardComparison}>
                                {this.props.unit === "standard"
                                  ? parseFloat(
                                      Math.abs(this.state.weightComparison) *
                                        2.20462
                                    ).toFixed(1)
                                  : parseFloat(
                                      Math.abs(this.state.weightComparison)
                      ...
                      <Expand value={this.state.isWeightCollapsed}>
                        <View style={{ height: 209, flex: 1 }}>
                          <MeasurementsChart
                            unit={this.props.unit === "standard" ? "lbs" : "kg"}
                            data={this.state.weightData}
                            minValue={
                              this.getMinValue(this.state.weightData) ===
                              this.getMaxValue(this.state.weightData)
                                ? 0
                                : this.getMinValue(this.state.weightData)
                            }
                            maxValue={this.getMaxValue(this.state.weightData)}
                          />
                        </View>
                      </Expand>
                    </View>
                  ...
                  >
                    <View>
                      <TouchableWithoutFeedback
                        onPress={() =>
                          this.setState({
                            isHeightCollapsed: !this.state.isHeightCollapsed,
                          })
                        }
                      >
                        <View
                        ...
                        >
                          <View>
                            ...
                            <Text style={styles.cardText}>
                              {this.props.unit === "standard"
                                ? parseFloat(
                                    this.state.heightCurrent / 2.54
                                  ).toFixed(1)
                                : this.state.heightCurrent}{" "}
                              <Text style={[styles.cardText, { fontSize: 14 }]}>
                          ...
                          >
                            ...
                            {this.state.heightComparison !== 0 && (
                              <Text style={styles.cardComparison}>
                                {this.props.unit === "standard"
                                  ? parseFloat(
                                      Math.abs(this.state.heightComparison) /
                                        2.54
                                    ).toFixed(1)
                                  : parseFloat(
                                      Math.abs(this.state.heightComparison)
                      ...
                      <Expand value={this.state.isHeightCollapsed}>
                        <View style={{ height: 209, flex: 1 }}>
                          <MeasurementsChart
                            data={this.state.heightData}
                            unit={this.props.unit === "standard" ? "in" : "cm"}
                            minValue={
                              this.getMinValue(this.state.heightData) ===
                              this.getMaxValue(this.state.heightData)
                                ? 0
                                : this.getMinValue(this.state.heightData)
                            }
                            maxValue={this.getMaxValue(this.state.heightData)}
                          />
                        </View>
                      </Expand>
                    </View>
...
export default BodyMetricsFullBody;
